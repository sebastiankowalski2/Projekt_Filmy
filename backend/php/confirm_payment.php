...
try {
    if ($paymentMethod === 'karta') {
        ...
        // Zaktualizowanie statusu wypożyczenia na "odbiór"
        $stmt = $conn->prepare("UPDATE wypozyczenia SET status = 'odbiór' WHERE id_uzytkownika = ? AND id_produktu = ? AND status = 'oczekuje' LIMIT 1");
        $stmt->bind_param("ii", $userId, $movieId);
        $stmt->execute();
        $stmt->close();
    } else {
        // Zaktualizowanie metody płatności na "gotówka" i statusu na "odbiór"
    ...
    echo json_encode(["success" => true, "message" => "Status wypożyczenia zmieniony na 'odbiór' i status płatności zmieniony na 'opłacono'."]);
} catch (Exception $e) {
